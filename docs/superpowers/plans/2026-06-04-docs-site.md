...
**Goal:** Add an Astro + Starlight documentation site at `apps/web`, deployed to GitHub Pages via GitHub Actions, in a Turborepo-managed pnpm monorepo.

**Architecture:** The repo becomes a pnpm workspaces monorepo with Turborepo orchestrating builds. `apps/web` is a standalone Astro + Starlight site with content authored directly in `src/content/docs/`. Biome (via `@repobuddy/biome`) provides linting and formatting. GitHub Actions deploys the built `dist/` to GitHub Pages on every push to `main` that touches `apps/web/**`.

**Tech Stack:** Astro 5, `@astrojs/starlight` 0.33, Turborepo 2, pnpm workspaces, Biome 1.9, `@repobuddy/biome`, GitHub Actions
...
| File | Action | Purpose |
|---|---|---|
| `package.json` | Create | Root workspace scripts + devDependencies |
| `pnpm-workspace.yaml` | Create | pnpm workspaces config |
| `turbo.json` | Create | Turborepo pipeline (build, dev) |
| `biome.json` | Create | Biome config extending @repobuddy/biome |
...
| `.github/workflows/deploy-docs.yml` | Create | CI/CD: build + deploy to GitHub Pages |
...
Expected: `node_modules/` and `pnpm-lock.yaml` created at root. No errors.

- [ ] **Step 6: Verify biome runs**

```bash
pnpm check 2>&1 | head -10
```

Expected: biome check starts up (no files to lint yet — that's fine). No crash.

- [ ] **Step 7: Commit**

```bash
git add package.json pnpm-workspace.yaml turbo.json biome.json pnpm-lock.yaml
git commit -m "chore: set up pnpm workspaces + Turborepo + Biome monorepo"
```
...
Expected: `apps/web/node_modules/` populated. `pnpm-lock.yaml` updated. No errors.

- [ ] **Step 5: Verify Astro can start (build will fail until content exists — that's expected)**

```bash
pnpm --filter web exec astro check 2>&1 | head -20
```

Expected: `astro check` starts up. Errors about missing content files are fine at this stage.

- [ ] **Step 6: Commit**

```bash
git add apps/web/package.json apps/web/astro.config.mjs apps/web/tsconfig.json pnpm-lock.yaml
...
```bash
pnpm --filter web exec astro build 2>&1 | tail -20
```

Expected: build completes (will warn about missing sidebar pages — fine until Tasks 4–7 are done). No fatal errors on the index page itself.
...
```bash
pnpm --filter web exec astro build 2>&1 | grep -E "(overview|error|Error)" | head -10
```
...
```bash
pnpm --filter web exec astro build 2>&1 | grep -E "(spec-lifecycle|error|Error)" | head -10
```
...
```bash
pnpm --filter web exec astro build 2>&1 | grep -E "(principles|error|Error)" | head -10
```
...
```bash
pnpm --filter web exec astro build 2>&1 | grep -E "(create-spec|error|Error)" | head -10
```
...
```bash
pnpm --filter web exec astro build 2>&1 | tail -10
```

Expected: build completes with no errors. All 7 routes built.

- [ ] **Step 3: Run biome check on the whole repo**

```bash
pnpm check 2>&1 | tail -10
```

Expected: biome check passes (or reports only whitespace/style issues, not errors). Fix any reported issues before committing.
